...
Read pending_review.json, extract YouTube transcripts and web page content,
output per-keyword JSON files in merged/.
"""
...
async def run_extract(project: str):
    """Run the extract step programmatically."""
    Config.init(project)

    print("=" * 70)
    print(f"  Step 2: Extract [{project}]")
    print("=" * 70)
    ...
    for kw_data in data["keywords"]:
        ...
        # Skip keywords that already have merged files
        keyword_file = keyword_to_filename(keyword)
        merged_path = f"{merged_dir}/{keyword_file}.json"
    ...
    if skipped_count > 0:
        print(f"\n⏭️  Skipped {skipped_count} keywords (merged file exists)")
    ...
    print(f"\nSaved: {total_saved} files → {merged_dir}/")
    print("=" * 70)


async def main():
    parser = argparse.ArgumentParser(description="Step 2: Extract content")
